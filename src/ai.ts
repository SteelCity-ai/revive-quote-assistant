...
export type AIReport={summary:string;items:AIItem[];materials:{itemIndex:number;product:string;supplier:string;location:string;preference:'local'|'online'|'unverified';availability:string;sourceId:string|null}[];benchmarks:{label:string;kind:'documented-project'|'cost-guide';location:string;low:number;high:number;unit:'sq ft'|'total';projectArea:number|null;scopeMatch:'whole-project'|'component'|'context-only';sourceId:string;notes:string}[];marketRange?:{low:number;high:number;basis:string}|null;assumptions:string[];exclusions:string[];questions:string[];researchGaps:string[];sources:Source[];researchedAt:string;model:string;area:number|null;researchNotes:string;fingerprint:string;lineIds:string[];resolvedQuestions:number[]};
...
export type RoofMeasure={available:boolean;reason?:string;formattedAddress?:string;roofAreaSqFt?:number;footprintSqFt?:number|null;coveredSqFt?:number|null;coveragePercent?:number|null;partial?:boolean;note?:string;imageryDate?:string|null;imageryQuality?:string;};
