...
export const voiceTools=[
  ...
  {type:'function',name:'measure_roof',description:'Fetch a pending roof measurement from Google aerial imagery for a roofing quote. It does not change the quote. Read back the address, area, imagery date and any coverage warning, then ask the user to confirm the correct building and included roof sections. If a roof area already exists, set replaceExisting true only after asking whether to remeasure.',parameters:{type:'object',properties:{address:{type:'string',description:'The job address to measure. Omit to use the address already recorded on the quote.'},replaceExisting:{type:'boolean',description:'True only when the user explicitly asked to replace or remeasure an existing roof area.'}},required:[],additionalProperties:false}},
  {type:'function',name:'confirm_roof_measurement',description:'Commit the pending Google roof measurement only after the user explicitly confirms it is the correct building and includes only the roof sections being quoted.',parameters:{type:'object',properties:{},required:[],additionalProperties:false}},
  {type:'function',name:'discard_roof_measurement',description:'Discard a pending Google roof measurement when the user rejects it, is unsure, or wants a manual measurement.',parameters:{type:'object',properties:{},required:[],additionalProperties:false}},
  {type:'function',name:'confirm_recap',description:'Confirm the four-answer recap only after the user says the recap is correct. The app blocks the next intake question until this tool succeeds.',parameters:{type:'object',properties:{},required:[],additionalProperties:false}},
  {type:'function',name:'correct_recap',description:'The user says something in the pending four-answer recap is wrong. Select the field to revisit before asking for the corrected answer.',parameters:{type:'object',properties:{field:{type:'string',description:'The question id from the pending recap that needs correction.'}},required:['field'],additionalProperties:false}},
...
export function buildSessionConfig({model,voice,context,transcriptionModel='gpt-4o-mini-transcribe'}){
  const instructions=[coreInstructions(),context?`APP CONTEXT:\n${context}`:''].filter(Boolean).join('\n\n');
  return {session:{type:'realtime',model,instructions,tools:voiceTools,tool_choice:'auto',audio:{output:{voice},input:{transcription:{model:transcriptionModel,language:'en'},turn_detection:{type:'semantic_vad',eagerness:'medium',create_response:true,interrupt_response:true}}}}};
}
...
export function createVoiceHandler({env=process.env,fetcher=fetch,authenticate,origins}){
  ...
  const transcriptionModel=env.OPENAI_REALTIME_TRANSCRIPTION_MODEL||'gpt-4o-mini-transcribe';
