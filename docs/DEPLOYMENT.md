...
- Voice: the server calls `https://api.openai.com/v1/realtime/client_secrets` directly (Headroom returns 404 for that path) and the browser connects WebRTC to `https://api.openai.com/v1/realtime/calls` (allowed by the production CSP). Optional env: `OPENAI_REALTIME_MODEL` (default `gpt-realtime-2.1`), `OPENAI_REALTIME_VOICE` (default `marin`), `OPENAI_REALTIME_TRANSCRIPTION_MODEL` (default `gpt-4o-mini-transcribe`), and `OPENAI_REALTIME_BASE_URL`.
- TypeSafe Jev is optional and server-side. Add `TYPESAFE_API_KEY`, `TYPESAFE_MODEL=jev-1.13.0`, and `TYPESAFE_VOICE_CONFIDENCE=0.65` to `/docker/revive-quote-assistant/shared/.env`. The app calls `https://api.typesafe.ai/v1/systemone` directly because Headroom does not implement it. Never put this key in a `VITE_` variable. Without the key, `/api/app/config` reports `jevAvailable:false` and voice continues through the existing Realtime path.
...
4. Run the image as a temporary staging container on loopback 4183 with production settings. Check `/healthz`, `/api/app/config`, static assets, anonymous AI/voice/roof/Jev denial, and portal capability connectivity. If a TypeSafe key is present, send one authenticated non-consequential `resume` classification probe and verify the pinned model/confidence response. Stop the temporary container after verification.
