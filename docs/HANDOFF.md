...
1. Conversational voice: **implemented on `feat/voice-conversation` (2026-09-21), merged into main, not deployed.** OpenAI Realtime over WebRTC with server-minted ephemeral tokens, semantic VAD, strict tools, validated answers, verbatim read-back + fingerprint-tied approval on the idempotent portal path. Headroom cannot carry the realtime transport (404 on `/v1/realtime/client_secrets`, verified 2026-09-21) so voice mints directly against api.openai.com while research/estimate keep Headroom routing; see `docs/VOICE.md`. Real-device microphone/Bluetooth/background checks and deployment remain outstanding — nothing voice-related is "verified working" until then.
2. Google roof measurement: implemented on main and being corrected against the live Solar API (real response structure verified from the VPS on 2026-09-21: `buildingInsights:findClosest`, `wholeRoofStats.areaMeters2` for sloped area, `buildingStats.groundAreaMeters2` / `wholeRoofStats.groundAreaMeters2` for footprint/coverage, coverage-percentage warning when partial). Key currently rejects Geocoding API requests — the key's API restriction must include Geocoding API before staging verification. Real-key staging check still required before release.
