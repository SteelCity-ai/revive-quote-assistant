...
**Talk to Revive** is implemented with the OpenAI Realtime API over WebRTC, using the GA flow from the official docs: the server mints a short-lived client secret (`POST /v1/realtime/client_secrets`, `OpenAI-Safety-Identifier` bound to a hash of the portal identity), and the browser connects directly to `POST /v1/realtime/calls` with that ephemeral token. The permanent key never leaves the server. Sessions use medium-eagerness `semantic_vad`, input transcription, barge-in, and sixteen strict function tools. The tool set adds deterministic four-answer recap confirmation and pending Google measurement confirmation to the intake, navigation, estimate and revision-tied approval tools.
...
Conversation flow is driven by app context: after every tool call the client sends a fresh context item (signed-in user, quote state, answers already provided, the exact next question) and a `response.create`, so Revive asks the existing domain questions one at a time and never re-asks provided answers. A committed-turn watchdog issues one recovery response after 4.5 seconds if Realtime fails to start one; timers are cancelled on response, pause, end, disconnect and teardown. Function-call IDs remain deduplicated across the session so a late `response.done` cannot repeat an earlier mutation. Every model answer passes through the existing `validAnswer` rules before touching the draft. Four-answer recaps block further intake until confirmed or corrected. Approval requires `prepare_approval` and then `confirm_approval` with the unchanged revision fingerprint; customer acceptance stays a portal action.

Configuration: `OPENAI_REALTIME_MODEL` (default `gpt-realtime-2.1`), `OPENAI_REALTIME_VOICE` (default `marin`), `OPENAI_REALTIME_TRANSCRIPTION_MODEL` (default `gpt-4o-mini-transcribe`), and `OPENAI_REALTIME_BASE_URL` (default `https://api.openai.com/v1`). Optional TypeSafe routing uses server-only `TYPESAFE_API_KEY`, pinned `TYPESAFE_MODEL=jev-1.13.0`, and `TYPESAFE_VOICE_CONFIDENCE=0.65`. `POST /api/voice/decision` is exact-origin, authenticated, size-bounded and per-identity limited. It sends only the current transcript/question/pause state to TypeSafe and returns a closed-set intent plus confidence. Jev is a hint for recovery/clarification only; it never prices, calculates, measures, approves, saves, or generates text. Both Realtime and TypeSafe are narrow direct-provider exceptions because Headroom does not implement their endpoints.

Google roof measurement is implemented as a proposal. The result discloses address, imagery date/quality and partial coverage, remains outside the quote until explicit voice confirmation, then records `Measured roof surface area`. Manual area/pitch entry remains available. `wholeRoofStats.areaMeters2` is already sloped surface area, so pitch is removed when a Google measurement is committed and is never applied twice. Waste stays separate.
